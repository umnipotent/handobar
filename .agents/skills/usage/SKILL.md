...
5. **OCP(개방-폐쇄 원칙) 유지와 확장성**
   ...
   - provider 고유 정보는 `UsageWindow`(예: 윈도우별 `chips`)나 `UsageSnapshot`의 `Option` 필드로만 싣고, 렌더링에 필요한 속성은 `UsageProvider` 디스크립터 인터페이스를 통해 필요할 때만 선택적 함수(`windowLabel`/`windowHint`/`defaultCollapsed`/`showModelBadges` 등)로 꽂아서 사용하도록 설계하는 방식이 유지보수에 유리하다.
