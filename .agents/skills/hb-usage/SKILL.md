...
커맨드 등록·invoke·ACL의 일반 패턴은 [`hb-tauri`](../hb-tauri/SKILL.md) 스킬을 따른다.
...
- 토큰 없음/만료/`401` → 한국어 로그인 안내. 첫 실행 시 macOS 키체인 프롬프트(서명은 [`hb-tauri`](../hb-tauri/SKILL.md) 코드 서명 절).
...
- [`hb-testing`](../hb-testing/SKILL.md): Rust/Vitest 테스트 작성·실행, 캐시 상태 격리, fake timer/localStorage mock.
- [`hb-tauri`](../hb-tauri/SKILL.md): 커맨드 등록 / invoke / ACL / macOS 코드 서명(키체인 접근).
