...
테스트 작성 원칙과 flaky/racy 테스트 대응은 [`hb-testing`](../testing/SKILL.md) 스킬을 따른다.
...
- [`hb-usage`](../usage/SKILL.md): provider별 잔여 사용량 추적이 이 커맨드 등록 / invoke / ACL 패턴 위에 올라간다.
  키체인에서 읽는 Claude 토큰이 코드 서명 프롬프트의 대상이다.
- [`hb-testing`](../testing/SKILL.md): Rust/Vitest 테스트 작성·실행, cache/fake timer/localStorage 상태 격리.
- [`hb-version`](../version-bump/SKILL.md): `tauri.conf.json` · `Cargo.toml` 의 `version` 동기화.

## 유닛 테스트

Rust/Vitest 테스트 작성·실행, cache/fake timer/localStorage 상태 격리, flaky/racy 테스트 대응은
[`hb-testing`](../testing/SKILL.md) 스킬을 단일 출처로 따른다.
