...
3. **트레이 상주 생명주기**: macOS에서 Accessory activation policy를 적용해 Dock 아이콘·Cmd+Tab에서 빠지는 순수 메뉴바 상주 앱으로 동작한다(Dock·Cmd+Q 종료 수단 자체가 없음). 창 닫기는 숨김 처리되고, 완전 종료는 트레이 메뉴 "종료"(`app.exit(0)`)로만 가능하다. 동작 변경 시 깨지지 않도록 [`hb-tauri` 스킬](.agents/skills/hb-tauri/SKILL.md)의 생명주기 섹션을 따른다.
