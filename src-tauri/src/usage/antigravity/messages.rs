...
pub(super) const CREDENTIALS_NOT_FOUND: &str =
    "Antigravity에 로그인되어 있지 않습니다. Antigravity IDE에서 로그인 후 다시 시도해 주세요.";
pub(super) const UNAUTHORIZED: &str =
    "Antigravity API 인증에 실패했습니다. Antigravity IDE에서 다시 로그인해 주세요.";
...
pub(super) fn token_refresh_failed(error: impl std::fmt::Display) -> String {
    format!("Antigravity OAuth 토큰 갱신 실패: {error}")
}
